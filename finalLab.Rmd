---
title: "w203 Lab 2: Understanding the impact of product variables on profit"
...
In our Lab 2, our team wanted to investigate the relationship between the order quantity and profit derived from sales at an unspecified store. This relationship could help the public understand the general store profitability, but more importantly, to understand the relationship of store profit to certain variables like product categories, customer segments, and order quantities. Different retailers take on different sales strategies. 

A real-world familiar example of this is Costco; large American retailer, Costco, often sells certain consumer goods (e.g. groceries or office supplies) at a discounted price and negative margin to get customer foot traffic. Once customers are enticed by these eye-catching, low-priced goods, they often engage with higher-margin items (e.g., consumer electronics) that are placed along the way to the lower-priced items of the store. 

Our team wanted to understand interesting relationships exist within the store of our chosen data set. Finding correlations was our metric of success. 

## Data and Research Design

Our data source from a Sales Store Product Details dataset in CSV format on Kaggle found at this link: https://www.kaggle.com/anuvagoyal/sales-store-product-details. These types of data sets are studied to find out the patterns in the selling structure and profit earned from them. 
The data set contains various details of products sold at a store. The data set has 13 columns describing product and sales data, no missing values, and over 700 rows. 

Because of its structure, this data set required limited cleaning; it has 13 well-defined variables to assist us in building our model including Profit, Order Quantity, Product Category, Product Sub Category, Product Name, and Customer Segment. Further analysis was done to ensure the “other” categories for Product Category and Customer Segment are clean data entries.

Formally, our research question is as follows:
What effects do variables like 'order quantity,' 'product category,' and 'customer segment' have on profit? 
...
The p-value of Model 1 shows that the 'Order Quantity' is significant when determining the profit. The estimated sales quantity coefficient is 0.002, which means that for every change in the order quantity, the profit increases by the following: (0.002 * the change in 'order quantity' * 100%). If the order quantity increased by 1, the profit would increase by 0.2%. We ensured not to include N/A values here.

It is important to note the R-squared value of 0.03 is relatively low meaning that our confidence in the linear model should not be extremely high because it doesn't do an insanely great job of explaining the data.
...
First, we determined all possible values in our categorical columns. 
...
We can see that model_office does a better job at predicting profit than model_product because model_profit has 2 insignificant variables (if_technology and if_furniture).

Now, we'll do the t-tests between these two models (model_1 and model_office). We dropped model_product. 
...
```{r results = 'asis'}
stargazer(model_1, model_office, model_office_interaction, align = TRUE, type = "text")
```
...
For our first model, we evaluated all of its assumptions. 
...
To answer our research question, we determined that for every change in the order quantity, the profit increases by (0.004 * the change in 'order quantity' * 100%). If office supplies are purchased, the profit increases by (0.06 * 100%) + (-0.0035 * change in order quantity if any * 100%). The other product categories or customer segments have no significant effect on the profit.
