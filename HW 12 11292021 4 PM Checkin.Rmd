...
pViews <- ggplot(data = d, aes(x = (views)) ) +
  ...
  labs(title = "Distribution of video views", x = "Views", y = "Count") +
...
```
> '
With a quick analysis, we have found the following insights for the views variable:
...
We have found the following insights for the average rating variable:
...
We have found the following insights for the length variable:
...
firstplot <- d_no_na %>%
  ...
  labs(title = 'Relationship between Average Rating and Model Residuals', subtitle = 'Data is distributed')
firstplot
secondplot <- d_no_na %>%
  ...
  scale_y_continuous(name = 'Model Residuals', labels = scales::comma) +
  labs(title = 'Relationship between Log of Video Length and Model Residuals', subtitle = 'Data is distributed')
secondplot
```
```{r}
#rlang::last_error()
```
> 4. **Homoskedastic Errors:** 'To assess whether the distribution of the errors is homoskedastic, we can examine the scale-location plot. Homoskedasticity would show up on this plot as a flat smoothing curve. The curve is linear with a slope of about 0, meaning our model satisfies the assumption of homoskedastic errors. We also performed the Breush Pagan Test, which yields a p-value of 2.2x10^-16, meaning we can reject the null hypothesis and conclude that there is no evidence of homoskedastic variance.'
...
errorplot <- d_no_na %>%
  ...
  scale_y_continuous(name = 'Model Residuals', labels = scales::comma) +
  labs(title = 'Homoskedastic Error Observation')
errorplot
lmtest::bptest(model)
```
> 5. **Normally Distributed Errors:** 'When assessing the normality of your error distribution, we're looking for the histogram of residuals and the qqplot to show a normal relationship. In this plot, you can see that errors seem to be normally distributed, meaning that our model satisfies the assumption of normally distributed errors.'
